...
public class DepthFirstSearch {
    ...
    public static void DFS(ArrayList<Edge> graph[], boolean visited[], int curr) {
        ...
        // Iterate through all edges (neighbors) of the current vertex
        for (int i = 0; i < graph[curr].size(); i++) {
            Edge e = graph[curr].get(i); // Get the edge from current vertex

            // If the destination vertex has not been visited
            if (!visited[e.des]) {
                // Recursively call DFS for the destination vertex
                DFS(graph, visited, e.des);
            }
        }

        // When all neighbors are visited, recursion unwinds
    }
